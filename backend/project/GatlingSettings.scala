...
object GatlingSettings {
  ...
  lazy val gatlingRunnerEcrSettings = Seq(
    ...
    repositoryName in Ecr := s"$dockerRemoteRepository/$projectBaseName-gatling-runner",
  ...
  lazy val gatlingAggregateRunnerEcrSettings = Seq(
    region in Ecr := Region.getRegion(Regions.AP_NORTHEAST_1),
    repositoryName in Ecr := s"$dockerRemoteRepository/$projectBaseName-gatling-aggregate-runner",
    localDockerImage in Ecr := (packageName in Docker).value + ":" + (version in Docker).value,
    push in Ecr := ((push in Ecr) dependsOn (publishLocal in Docker, login in Ecr)).value
  )
      ...
      client.listTaskDefinitions(request).flatMap { result =>
        if (result.sdkHttpResponse().isSuccessful) {
          result.nextTokenAsScala match {
            case None =>
              val arns = result.taskDefinitionArns().asScala
              logger.info(s"arns = $arns")
              Future.successful(arns.head)
            case Some(nextToken) =>
  ...
  val gatlingAggregateRunTaskSettings = Seq(
    ...
    runTaskTaskDefinition in gatling := {
      ...
      getTaskDefinitionName(
        ...
        prefix = sys.env.getOrElse(
          "GATLING_ECS_AGGREGATE_RUNNER_TASK_DEFINITION",
      ...
      Map(
        ...
        "GATLING_TASK_DEFINITION" -> {
          ...
          getTaskDefinitionName(
            client = (runTaskEcsClient in gatling).value,
            awaitDuration = (runTaskAwaitDuration in gatling).value,
            prefix = sys.env.getOrElse("GATLING_ECS_RUNNER_TASK_DEFINITION", "j5ik2o-aws-gatling-tools-gatling-runner")
          )
        },
        "GATLING_COUNT"               -> sys.env("GATLING_COUNT"),
        "GATLING_ENABLE_READ"         -> sys.env("GATLING_ENABLE_READ"),
        "GATLING_PAUSE_DURATION"      -> sys.env("GATLING_PAUSE_DURATION"),
        "GATLING_READ_PAUSE_DURATION" -> sys.env("GATLING_READ_PAUSE_DURATION"),
        "GATLING_RAMP_DURATION"       -> sys.env("GATLING_RAMP_DURATION"),
        "GATLING_HOLD_DURATION"       -> sys.env("GATLING_HOLD_DURATION"),
        "GATLING_GRPC_WRITE_HOST" -> sys.env(
          "GATLING_GRPC_WRITE_HOST"
        ),
        "GATLING_GRPC_WRITE_PORT" -> sys.env(
          "GATLING_GRPC_WRITE_PORT"
        ),
        "GATLING_GRPC_READ_HOST" -> sys.env(
          "GATLING_GRPC_READ_HOST"
        ),
        "GATLING_GRPC_READ_PORT" -> sys.env(
          "GATLING_GRPC_READ_PORT"
        ),
        ...
        "GATLING_REPORTER_TASK_DEFINITION" -> {
          ...
          getTaskDefinitionName(
            ...
            awaitDuration = (runTaskAwaitDuration in gatling).value,
            prefix = sys.env
              .getOrElse("GATLING_ECS_S3_REPORTER_TASK_DEFINITION", "j5ik2o-aws-gatling-tools-gatling-s3-reporter")
          )
        },
    ...
    runTask in gatling := {
      ...
      result.foreach { task: Task => log.info(s"task.arn = ${task.taskArn()}") }
