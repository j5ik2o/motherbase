...
object NoticeUtil {
  ...
  def sendMessageToChatwork(host: String, roomId: String, token: String, message: String): HttpResponse[String] = {
    val url = s"https://$host/v2/rooms/$roomId/messages"
  ...
  def sendMessageToChatwork(
    ...
    (hostOpt, roomIdOpt, tokenOpt) match {
      case (Some(h), Some(r), Some(t)) if h.nonEmpty && r.nonEmpty && t.nonEmpty =>
        val response = sendMessageToChatwork(h, r, t, message)
        logger.info(s"sendMessageToChatwork.response = $response")
      case _ =>
        logger.info("disabled chatwork notice")
    }
  }

  def sendMessagesToSlack(incomingWebhookUrlOpt: Option[String], message: String): Unit = {
    incomingWebhookUrlOpt match {
      ...
      case _ =>
        logger.info("disabled slack notice")
    }
  }
}
