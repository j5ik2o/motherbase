...
object Runner extends App {
  ...
  val simulationClassName =
    config.getString("motherbase.gatling.simulation-classname")
  val executionId = config.getString("motherbase.gatling.execution-id")

  val s3EndPoint = {
    val endPoint = config.getString("motherbase.gatling.aws-s3-endpoint")
    if (endPoint.isEmpty) None else Some(endPoint)
  }
  val bucketName = config.getString("motherbase.gatling.aws-s3-bucket-name")

  val createBucketOnStart =
    config.getBoolean("motherbase.gatling.aws-s3-create-bucket-on-start")

  val pathStyleAccess =
    config.getBoolean("motherbase.gatling.aws-s3-path-style-access")
