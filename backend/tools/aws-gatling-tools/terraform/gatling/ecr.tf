...
EOF
  ...
  repository = element(concat(aws_ecr_repository.gatling_runner_ecr[*].name, list("")), 0)
  lifecycle {
    create_before_destroy = true
  }
}
...
EOF
  ...
  repository = element(concat(aws_ecr_repository.gatling_s3_reporter_ecr[*].name, list("")), 0)
  lifecycle {
    create_before_destroy = true
  }
}
